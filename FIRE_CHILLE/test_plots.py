...
# cols = ['date', 'Algae Conc']
df = pd.read_csv("climate_data.csv", index_col='date')

df_copy = df.copy()
df_copy.rename(columns={'value': 'Temperature (K)'}, inplace=True)


time = df_copy.index()

df_c = df_copy.copy()
alg = df_c['Temperature (K)']
algae = []
# for a in alg:
#     if str(a) == 'nan':
#         x = np.float64(0)
...
# df_copy = df_copy.drop('Temperature (K)', axis = 1)
...
for t in time:
    # print(t)
    dat = dt.strptime(t, "%d-%b-%yT%H%M%S")
    time_s = dt.strftime(dat, "%d-%b-%y")
    # if dat.month == 3:
...
plt.title("NCBC Temperature (K) (1988-2021)", fontsize=40)
